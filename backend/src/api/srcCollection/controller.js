...
import { FindProcessCreateCollection, FindProcessIdCreateCollection } from '../provider/controller'

export const create = ({ bodymen: { body } }, res, next) => SrcCollection.create({ ...body, status: 'created' })
...
export const importCollections = async (req, res, next) => {
  FindProcessCreateCollection()
    .then(provider => getCollectionsAll(provider)
      .then(collections => success(res, 201)(collections))
    )
}

export const index = ({ querymen: { query, select, cursor } }, res, next) => SrcCollection.count(query)
  .then(count => SrcCollection.find(query, select, cursor)
    .then((srcCollections) => ({
      count, rows: srcCollections.map((srcCollection) => srcCollection.view())
    })))
  .then(success(res))
  .catch(next)

export const show = ({ params }, res, next) => SrcCollection.findById(params.id)
  .then(notFound(res))
  .then((srcCollection) => srcCollection ? srcCollection.view() : null)
  .then(success(res))
  .catch(next)

export const update = ({ bodymen: { body }, params }, res, next) => SrcCollection.findById(params.id)
  .then(notFound(res))
...
export const destroy = ({ params }, res, next) => SrcCollection.findById(params.id)
  .then(notFound(res))
  .then((srcCollection) => srcCollection ? srcCollection.remove() : null)
  .then(success(res, 204))
  .catch(next)

export const changeStatus = (id, status) => SrcCollection
  .findById(id)
  .then((srcCollection) => srcCollection ? Object.assign(srcCollection, { status }).save() : null)

export const CreateCollection = async (collection) => {
  const provider = await FindProcessIdCreateCollection()
  const {
    name, slug, detail, totalAssets
  } = collection
  ...
  if (exist) {
    ...
    return await SrcCollection.create({ ...collection, provider, status: 'created' })
  }
}

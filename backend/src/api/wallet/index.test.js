...
beforeEach(async () => {
  ...
  wallet = await Wallet.create({ user })
})

test('POST /wallets 201 (user)', async () => {
  const { status, body } = await request(app())
    .post(`${apiRoot}`)
    .send({ access_token: userSession, name: 'test', secret: 'test' })
  expect(status).toBe(201)
  expect(typeof body).toEqual('object')
  expect(body.name).toEqual('test')
  expect(body.secret).toEqual('test')
...
test('PUT /wallets/:id 401 (user) - another user', async () => {
  const { status } = await request(app())
    .put(`${apiRoot}/${wallet.id}`)
    .send({ access_token: anotherSession, name: 'test', secret: 'test' })
  expect(status).toBe(401)
})

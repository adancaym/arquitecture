...
export const dispatched = ({ body: { order }, params: { id } }, res, next) =>
  Placement.findById(id)
    .then(notFound(res))
    .then((placement) => placement ? Object.assign(placement, { order, status: 'dispatched' }).save() : null)
    .then((placement) => placement ? placement.view(true) : null)
    .then(success(res))
    .catch(next)

export const dispatchedError = ({ body: { error }, params: { id } }, res, next) =>
  Placement.findById(id)
    .then(notFound(res))
    .then((placement) => placement ? Object.assign(placement, { error, status: 'error' }).save() : null)
    .then((placement) => placement ? placement.view(true) : null)
    .then(success(res))
    .catch(next)

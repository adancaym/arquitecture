...
beforeEach(async () => {
  ...
  user = await User.create({ email: 'adanyanezgonzalez@gmail.com', password: '123456' })
...
test('POST /password-resets 202 (master)', async () => {
  const { status } = await request(app())
    .post(apiRoot)
    .send({ access_token: masterKey, email: 'adanyanezgonzalez@gmail.com', link: 'http://example.com' })
...
test('POST /password-resets 400 (master) - missing link', async () => {
  const { status, body } = await request(app())
    .post(apiRoot)
    .send({ access_token: masterKey, email: 'adanyanezgonzalez@gmail.com' })
  expect(status).toBe(400)
  expect(typeof body).toBe('object')
...
test('POST /password-resets 401', async () => {
  const { status } = await request(app())
    .post(apiRoot)
    .send({ email: 'adanyanezgonzalez@gmail.com', link: 'http://example.com' })

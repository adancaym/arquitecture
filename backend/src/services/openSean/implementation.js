...
import {
  CreateCollection,
  OpenseaCollectionToSrcCollection,
} from "../../api/srcCollection/controller";
import { FindKeyPublicKey } from "../../api/provider/controller";

const createAsset = (body) =>
  Asset.findOne({
    tokenId: body.tokenId,
    srcCollection: body.srcCollection,
  }).then(async (asset) =>
    asset ? await Object.assign(asset, body).save() : await Asset.create(body)
  );
const changeCollection = async ({ name, slug }, update) =>
  SrcCollection.findOneAndUpdate({ name, slug }, update);
const addTraitsToCollection = async (idCollection, traits) =>
  SrcCollection.findById(idCollection).then(async (collection) => {
    for (const trait of traits) {
      const existTrait = collection.traits.find(
        (traitInCollection) => traitInCollection.key === trait.trait_type
      );
      if (existTrait) {
...
const getAssets = async (collection, provider, cursor) => {
  ...
  return openSea(provider.urlBase)
    .getAssets(apikey, {
...
export const getAssetsForCollection = async (srcCollection) => {
  ...
  switch (provider.name) {
    case "opensea": {
      ...
      await getAssets(srcCollection, provider).then(() =>
        changeCollection(
          { name: srcCollection.name, slug: srcCollection.slug },
          { status: "populated" }
        )
      );
...
export const findLastOffer = (provider) => async (asset) => {
  ...
  events = events.filter(
    (cursor) => cursor.event.payment_token.symbol === "WETH"
  );
  const max = new Date(
    Math.max(...events.map((ae) => new Date(ae.event_timestamp)))
  );
  return events.find(
    (event) => new Date(event.event_timestamp).getTime() === max.getTime()
  );
};
let assetsEvents = [];
...
let collectionsPool = [];
const getCollectionsFromOpenSea = async (provider, offset = 0) =>
  openSea(provider.urlBase)
    .getCollectionsAll(offset)
    .then(async (response) => {
...
const storeCollectionsFromOpenSea = async (collections) =>
  Promise.all(
    collections
...
export const getCollectionsAll = async (provider) =>
  new Promise((resolve) => {
    switch (provider.name) {
      case "opensea": {
        return getCollectionsFromOpenSea(provider)
          .then(() => (collectionsPool = []))
          .then(() => resolve());
      }
    }
  });
